find_individual_genealogie <- function(url){
  ...
  geboorte <- gedoopt <- overleden <- naam <- ouders_data <- kinderen_data <- NA
  data <- read_html(url) |>
  ...
  onl <- tryCatch({data.frame(names = ouders_data |> html_text2(),
             links = paste0(naked_url, '/', ouders_data |> html_attr('href')))}, error=function(e) return(NA))
  ...
  knl <- tryCatch({data.frame(names = kinderen_names,
                    links = kinderen_links)}, error=function(e) return(NA))
